...
void config_timer(uint32_t timer_periph) {
    ...
    uint16_t every_x_milliseconds = 1000;
...
void TIMER2_IRQHandler(void)
{
    ...
    // we used channel 0 so we gotta clear this flag when it's active
    if (timer_interrupt_flag_get(TIMER2, TIMER_INT_FLAG_CH0) != RESET) {
        timer_interrupt_flag_clear(TIMER2, TIMER_INT_FLAG_CH0);
        /* our own reaction to channel 0 triggering: invert GPIO pin, blinky blink */
